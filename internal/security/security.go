...
func (rl *RateLimiter) Allow(identifier string) bool {
	...
	rl.cleanOldRequests(identifier, now)

	// Check if under limit
	currentRequests := len(rl.requests[identifier])
	if currentRequests >= rl.maxRequests {
		return false
	}

	// Add current request
	if rl.requests[identifier] == nil {
...
// GetStats returns rate limiting statistics for an identifier
func (rl *RateLimiter) GetStats(identifier string) RateLimitStats {
	rl.mu.RLock()
	defer rl.mu.RUnlock()
	...
	if !oldestRequest.IsZero() && oldestRequest.After(windowStart) {
		windowStart = oldestRequest
	}

	nextReset := windowStart.Add(rl.window)
	if !oldestRequest.IsZero() {
		nextReset = oldestRequest.Add(rl.window)
	}
	...
	return RateLimitStats{
		...
		WindowDuration: rl.window,
		MaxRequests:    rl.maxRequests,
		IsBlocked:      validRequests >= rl.maxRequests,
		NextResetTime:  nextReset,
		RemainingQuota: max(0, rl.maxRequests-validRequests),
	}
}
...
// cleanupRoutine periodically cleans up old entries to prevent memory leaks
...
// performCleanup removes old entries from all identifiers
func (rl *RateLimiter) performCleanup() {
	...
	for identifier := range rl.requests {
		...
		if len(rl.requests[identifier]) == 0 {
			delete(rl.requests, identifier)
		}
	}
}

// cleanOldRequests removes expired requests for a specific identifier
func (rl *RateLimiter) cleanOldRequests(identifier string, now time.Time) {
